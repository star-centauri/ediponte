...
function AccessPonte(ponte: TPonte): Boolean;
var
    access: String;
begin
    Result := false;
    ...
    Case ponte.Porta of
        21 : access := 'C:\Windows\System32\ftp.exe ' + ponte.Servidor;
        22 : access := 'psftp.exe ' + ponte.Conta + '@' + ponte.Servidor;
    else sintWrite('Acesso negado.');
    end;

    if executarAcesso(access) then
        Result := true;
end;
                ...
                while not baixarArq(nomeArq) do
                    begin
                        if tipoDeErro = ERRO_ACESSTERM then
                            begin
                                if not AccessPonte(ponteConectadaFTP) then
                                    begin
                                        ERRO := ERRO_CONEXAO;
                                        progStop;
                                        exit;
