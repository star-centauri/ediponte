...
begin
    ...
    repeat
        ...
        abreWinSock;
        if _findFirst(mascSelecao, atribArq, sr) = 0 then
            repeat
                if sr.Name <> '.' then
                    begin
                        new (psr);
            ...
            until _FindNext(sr) <> 0;
        _FindClose(sr);
        fechaWinSock;
    until mascMultipla = '';

    if listArq.Count = 0 then
        sintWriteLn('Diretório inacessível. Aperte ENTER.');
end;
            ...
            repeat
                ...
                if qualOpcao < listArq.count then
                    begin
                        ...
                        nome := semAcentos(ansiuppercase(psr^.sr.FindData.cFileName));
                        if noMeio then
        ...
        begin
            if dirPedido = dirAtual then
                ...
                if dirPedido [length(dirPedido)] <> '\' then
                    dirPedido := dirPedido + '\';
            selecArq := dirPedido + PMySearchRec(listArq[n]).sr.FindData.cFileName;
        end
    else
        selecArq := '';
